...
You can start writing assets in `datalized/assets/`. The assets are automatically loaded into the Dagster repository as you define them.
...
Tests are in the `datalized_tests` directory and you can run tests using `pytest`:

```bash
pytest datalized_tests
```
